...
var PushNotification = /** @class */ (function () {
				...
				// TODO: The service should provide data in a consistent format similar to iOS
					...
					eventSourceAttributes = {
						...
						campaign_activity_id:
							data['pinpoint.campaign.campaign_activity_id'],
	...
	PushNotification.prototype.configure = function (config) {
		if (core_1.isEmpty(config)) return this._config;
	...
	PushNotification.prototype.onNotificationOpened = function (handler) {
		if (typeof handler === 'function') {
			this._notificationOpenedHandlers = __spreadArrays(
				this._notificationOpenedHandlers,
				[handler]
			);
		}
					...
					case 1:
						...
						if (!_a.sent()) {
							...
							RNPushNotification.getToken(
								...
								function (error) {
									logger.error(
										'Error getting the token from Firebase Service',
										error
									);
								}
							);
	...
	PushNotification.prototype._registerTokenCached = function () {
		return __awaiter(this, void 0, void 0, function () {
			...
			return __generator(this, function (_a) {
				...
				return [
					2 /*return*/,
					async_storage_1.default.getItem(cacheKey).then(function (lastToken) {
	 ...
	 */
	PushNotification.prototype._checkIfOpenedByNotification = function (
		nextAppState,
		handler
	) {
	...
	PushNotification.prototype.handleNotificationReceived = function (
		rawMessage
	) {
		logger.debug('handleNotificationReceived, raw data', rawMessage);
		var _a = this.parseMessageData(rawMessage),
			eventSource = _a.eventSource,
			eventSourceAttributes = _a.eventSourceAttributes;
		if (!eventSource) {
		...
		var attributes = __assign(__assign({}, eventSourceAttributes), {
		...
		if (
			core_1.Amplify.Analytics &&
			typeof core_1.Amplify.Analytics.record === 'function'
		) {
	...
	PushNotification.prototype.addEventListenerForIOS = function (
		event,
		handler
	) {
		var _this = this;
		if (event === REMOTE_TOKEN_RECEIVED) {
			push_notification_ios_1.default.addEventListener(
				'register',
				function (data) {
					handler(data);
				}
	...
	PushNotification.prototype.parseMessagefromAndroid = function (
		message,
		from
	) {
		var dataObj = null;
